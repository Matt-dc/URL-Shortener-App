...
class Shortener extends Component {
    ...
    componentDidMount(){
      ...
      urlUpdateHandler = val => this.setState({ urlUpdater: val.target.value })
    
      setCopiedTrue = url => this.setState({ copied: url })
      
      setCopiedFalse = () => this.setState({ copied: null })
      ...
      handleShorten = e => {
        ...
        if(this.state.URL == '') {
          ...
          axios.post('http://localhost:8888/shorten', url)
          .then(res => {
              if(res.status === 200) {
                this.setState({
                ...
                if(err.response.status === 409) {
                  this.setState({
                    ...
                    this.setState({
                      ...
                      error: err.response.data.error,
                      disabled: true,
                      error: "That URL appears not to be valid :/ - it should begin with http or https",
                      loading: false,
                    })
                }
            })
    ...
    saveUrl = url => {
      ...
      let obj = this.state.shorteningHistory.find(item => item == url).shortUrl.split('/').pop()

      // this.setState({ uniqueCode: JSON.stringify(uniqueCode), urlUpdater: JSON.stringify(urlObj) })

      axios.put(`http://localhost:8888/update/${obj}`, { urlToUpdate: this.state.urlUpdater })
      .then(res => {
        this.setState({
          shorteningHistory: res.data.urls,
          editingUrl: null,
    ...
    confirmDeleteUrl = longUrl => {
      ...
      const urlToDelete = this.state.shorteningHistory.find(item => item.longUrl == longUrl).shortUrl.split('/').pop()

      axios.delete(`http://localhost:8888/delete/${urlToDelete}`)
      .then(res => {
        this.setState({
          shorteningHistory: res.data.urls,
          showDeleteModal: null,
        ...
        return (
            <>
                ...
                <Row style={{ margin: '0 0 4em 0' }}>
                  <Col>
                      <p style={{ fontSize: '1em', color: '#363636' }}>Paste in a URL and click the button to get a shortened version</p>
                      <p style={{ fontSize: '0.8em', color: '#6e6e6e' }}>( Please note, links only last 24 hours :/ )</p>
                  </Col>
                </Row>
                ...
                <Row>
                  ...
                  <Col md={1}></Col>
                   ...
                   <Row >
                        <Col md={1}></Col>
                        <Col md={10}>
                            {this.state.shorteningHistory.length > 0 && this.state.shorteningHistory.map(url => {
                            return (
                                <UrlHistoryItem
